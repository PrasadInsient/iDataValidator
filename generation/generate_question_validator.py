            ...
            with open(question_validator_file, 'w') as f:
                ...
                for question in questions:
                    ...
                    if match:
                        ...
                        f.write(f"  #{qid}\n  check_valid(eval(f'QUESTIONS.{match.group(1)}_{match.group(2)}{{loopid}}'),qtype=QUESTIONTYPES.NONE,valid_values=np.arange(0, 1), exclusive=[],allow_blanks=False,\n             condition =lambda row,loopid: True,range_value=(0,100))\n\n")
                    else:    
                        f.write(f"  #{qid}\n  check_valid(QUESTIONS.{qid},qtype=QUESTIONTYPES.NONE,valid_values=np.arange(0, 1), exclusive=[],allow_blanks=False,\n             condition =lambda row: True,range_value=(0,100))\n\n")
                f.write(f"'''\n\n")
