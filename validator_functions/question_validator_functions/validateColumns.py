...
def qvalidate_single_multiple(questionid, datacols, data, valid_values, optional_cols, at_least, at_most,allowblanks):
    ...
    for column in datacols:
        if callable(valid_values):
            data['valid_values'] = data.apply(lambda row,column: valid_values(row,column), axis=1)
        else:
...
def qvalidate_completeness(questionid, datacols, data, required, at_least, at_most):
    if required == 1:
        ...
        non_blank_counts = data[datacols].applymap(non_blank_condition).sum(axis=1)
        ...
        for index in data.index[fewer_than_min]:
            adderror(data.at[index, 'record'], questionid, "", f"At least resposnes check failed - {at_least} valid responses found.")
        ...
        for index in data.index[more_than_max]:
            adderror(data.at[index, 'record'], questionid, "", f"At most resposnes check failed - {at_least} valid responses found.")
