...
def checkrank(questionid: str, datacols: list, datarow: pd.Series, min_rank_value: int = 1, 
              max_rank_value: Optional[int] = None, exclude_cols: list = [], condition: bool = True):
    """
    ...
    Parameters:
        ...
        exclude_cols (list, optional): A list of column names to exclude from the rank check. These columns 
    ...
    # Filter columns to check based on exclude_cols
    columns_to_check = [col for col in datacols if col not in exclude_cols]
